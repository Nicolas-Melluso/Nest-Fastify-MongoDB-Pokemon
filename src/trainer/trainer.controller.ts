...
export class TrainerController {
    ...
    @Post('/create')
    async createTrainer(@Res() res: FastifyReply, @Body() createTrainerDTO: CreateTrainerDTO) {
    ...
    async getTrainerById(@Res() res: FastifyReply, @Param('id') trainerID) {
        ...
        if(!trainer) throw new NotFoundException('The Trainer does not exist yet, try in the next generation');
        res.status(HttpStatus.OK).send({
    ...
    async deleteTrainer(@Res() res: FastifyReply, @Query('id') trainerID) {
        ...
        if(!trainer) throw new NotFoundException('The Trainer you wanted to eliminate did not exist, you have nothing to worry about anymore');
        res.status(HttpStatus.OK).send({
    ...
    @Put('/:id')
    async updateTrainer(@Res() res: FastifyReply, @Body() createTrainerDTO: CreateTrainerDTO, @Param('id') trainerID) {
    ...
    async tryCatchPokemonByPokedexID(@Res() res: FastifyReply, @Param('id') trainerID, @Param('pokedexID') pokedexID) {
    ...
    async tryCatchPokemonRandom(@Res() res: FastifyReply, @Param('id') trainerID) {
    ...
    async releasePokemon(@Res() res: FastifyReply, @Param('trainerID') trainerID, @Param('pokemonID') pokemonID,) {
