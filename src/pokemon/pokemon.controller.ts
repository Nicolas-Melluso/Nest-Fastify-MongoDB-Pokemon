...
export class PokemonController {
    ...
    if (pokemonID.match(/^[0-9a-fA-F]{24}$/)) {
      ...
      if (!pokemon)
        throw new NotFoundException(
          'The pokemon does not exist yet, try in the next generation',
        );
      res.status(HttpStatus.OK).send({
  ...
  async deletePokemon(@Res() res: FastifyReply, @Query('id') pokemonID) {
    ...
    if (!pokemon)
      throw new NotFoundException(
        'The pokemon you wanted to eliminate did not exist, you have nothing to worry about anymore',
      );
    res.status(HttpStatus.OK).send({
  ...
  async updatePokemon(
    ...
    const pokemon = await this.pokemonService.updatePokemon(
      pokemonID,
      createPokemonDTO,
    );
    if (!pokemon)
